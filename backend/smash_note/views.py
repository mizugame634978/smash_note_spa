    ...
    def get(self, request):
        ...
        if self.request.user.is_authenticated:
            try:
                ...
                return render(
                    request,
                    "smash_note/favorite_characters.html",
                    {
                        "form": form,
                ...
                pass
        return render(
            request, "smash_note/favorite_characters.html", {"form": form, "character_choices": character_choices, "favorite_character": favorite_character}
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            return render(
                request,
                "smash_note/favorite_characters.html",
                {"form": form, "character_choices": character_choices, "favorite_character": favorite_character, "favorite_characters": favorite_characters},
            )
        except FavoriteCharacter.DoesNotExist:
            pass
        return render(
...
class ToolView(generic.ListView):  # 使用率、勝率の高い低いで3キャラ表示
    ...
    def get_context_data(self, **kwargs):
        ...
        sorted_indices = sorted(
            range(chara_num), key=lambda x: player_use_characters[x], reverse=True
        )  # 入っている数字が大きい順にソートしたもののp_u_cのインデックス
...
class UseRateView(generic.ListView):  # 使用率、勝率の高い低いで3キャラ表示
    ...
    def get_context_data(self, **kwargs):
        ...
        for i in range(1, characters_num + 1):  # リストは０から始まるがidは１から始まるため
            ...
            player_use_characters.append(tmp)
        sorted_indices = sorted(
            range(characters_num), key=lambda x: player_use_characters[x], reverse=True
        )  # 入っている数字が大きい順にソートしたもののインデックス
...
class WinRateView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        only_wining_rate = [
            -1
            if player_wins.get(character_id, 0) + player_losses.get(character_id, 0) == 0
            else (player_wins.get(character_id, 0) / (player_wins.get(character_id, 0) + player_losses.get(character_id, 0) or 1)) * 100
            for character_id in range(1, chara_num + 1)
        ]

        player_winning_rates = {character_id: only_wining_rate[character_id - 1] for character_id in range(1, chara_num + 1)}  # 勝率を計算する,{id,勝率}を生成
